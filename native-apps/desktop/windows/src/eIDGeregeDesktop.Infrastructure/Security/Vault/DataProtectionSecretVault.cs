...
namespace eIDGeregeDesktop.Infrastructure.Security.Vault;

public sealed class SecretVaultOptions
{
    public const string SectionName = "eIDGeregeDesktop:SecretVault";

    /// Optional override. Defaults to %LOCALAPPDATA%/eIDGeregeDesktop/secrets.bin.
    ...
    public string Entropy { get; init; } = "eIDGeregeDesktop.SecretVault.v1";
}
...
/// not provided (the eIDGeregeDesktop desktop client is single-instance).
...
public sealed class DataProtectionSecretVault : ISecretVault, IDisposable
{
    ...
    private static string DefaultPath()
    {
        ...
        return Path.Combine(local, "eIDGeregeDesktop", "secrets.bin");
    }
