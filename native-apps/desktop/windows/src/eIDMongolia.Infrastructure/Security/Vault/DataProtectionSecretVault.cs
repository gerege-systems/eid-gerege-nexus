...
namespace eIDMongolia.Infrastructure.Security.Vault;

public sealed class SecretVaultOptions
{
    public const string SectionName = "eIDMongolia:SecretVault";

    /// Optional override. Defaults to %LOCALAPPDATA%/eIDMongolia/secrets.bin.
    ...
    public string Entropy { get; init; } = "eIDMongolia.Desktop.SecretVault.v1";
}
...
/// not provided (the eIDMongolia desktop client is single-instance).
...
public sealed class DataProtectionSecretVault : ISecretVault, IDisposable
{
    ...
    private static string DefaultPath()
    {
        ...
        return Path.Combine(local, "eIDMongolia", "secrets.bin");
    }
