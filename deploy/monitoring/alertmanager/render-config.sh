...

{
    ...
    if [ -n "${ALERT_EMAIL_TO:-}" ] && [ -n "${ALERT_SMTP_HOST:-}" ]; then
        ...
        echo "        headers:"
        echo "          Subject: '[eID Gerege/{{ .CommonLabels.severity }}] {{ .CommonAnnotations.summary }}'"
    fi
    ...
    if [ -n "${ALERT_EMAIL_TO:-}" ] && [ -n "${ALERT_SMTP_HOST:-}" ]; then
        ...
        echo "        send_resolved: true"
        echo "        headers:"
        echo "          Subject: '[eID Gerege/PAGE] {{ .CommonAnnotations.summary }}'"
    fi
    if [ -n "${ALERT_TELEGRAM_TOKEN:-}" ] && [ -n "${ALERT_TELEGRAM_CHAT_ID:-}" ]; then
        ...
        echo "        send_resolved: true"
        echo "        message: '{{ template \"telegram.eid\" . }}'"
    fi
} >> "$OUT"
